...
@section('form_input')
    <div class="mb-1">
        <label class="form-label" for="period">{{trans('admin.period')}}</label>
        <input type="number" name="period" min="1"
               class="form-control dt-full-name"
               id="period"
               placeholder="{{trans('admin.period')}}"/>
    </div>

    <div class="mb-1">
        <label class="form-label" for="type">{{trans('admin.type')}}</label>
        <select id="type" class="select2 form-select" name="type">
            @foreach(\App\Entities\NegotiationPeriodType::getKeys() as $key => $type)
                <option value="{{$type}}">{{trans('admin.'.$type)}}</option>
            @endforeach
        </select>
    </div>
                    ...
                    <div class="card-header border-bottom">
                        <h4 class="card-title">
                            <button type="button" class="btn btn-primary" id="add_btn"
                                ...
                                {{trans('admin.add_negotiation_period')}}
                            </button>
                        </h4>
                    </div>
...
@section('page-script')
    ...
    <script>
        $(function () {
            ...
            addModal({
                title: '{{trans('admin.add_negotiation_period')}}',
            });

            onClose();

            loadDataTables('{{ url("/admin/negotiation/periods/data", [] , env('APP_ENV') === 'local' ?  false : true)}}',
                ['period', 'type', 'actions'], '',
                {
            ...
            $('#general-form').submit(function (e) {
                e.preventDefault();
                sendModalAjaxRequest(this, '{{url('/admin/negotiation/period/add', [] , env('APP_ENV') === 'local' ?  false : true)}}',
                    '{{url('/admin/negotiation/period/edit', [] , env('APP_ENV') === 'local' ?  false : true)}}', {
        ...
        function editNegotiationPeriod(item) {
            var id = $(item).attr('id');
            var form = new FormData();
            form.append('id', id);
            $('.modal-title').text('{{trans('admin.edit_negotiation_period')}}');
            pub_id = id;
            $.ajax({
                url: '{{url('/admin/negotiation/period/data', [] , env('APP_ENV') === 'local' ?  false : true)}}',
                method: 'POST',
                data: form,
                processData: false,
                contentType: false,
                ...
                success: function (response) {
                    $('#general-form input[name=period]').val(response.data.period);
        ...
        function deleteNegotiationPeriod(item) {
            ban(item, '{{url('/admin/negotiation/period/delete', [] , env('APP_ENV') === 'local' ?  false : true)}}', {
                error_message: '{{trans('admin.general_error_message')}}',
