...
@section('page-script')
    ...
    <script>
        $(function () {
            ...
            addModal({
                title: '{{trans('admin.add_committee')}}',
                dropify: true
            });

            onClose();

            loadDataTables('{{ url("/admin/committees/data") }}',
            ...
            $('#general-form').submit(function (e) {
                e.preventDefault();
                sendModalAjaxRequest(this, '{{url('/admin/committee/add')}}',
                    '{{url('/admin/committee/edit')}}', {
        ...
        function editCommittee(item) {
            ...
            form.append('id', id);
            $('.modal-title').text('{{trans('admin.edit_committee')}}');
            pub_id = id;
            $.ajax({
                url: '{{url('/admin/committee/data')}}',
                method: 'POST',
                data: form,
                processData: false,
                contentType: false,
        ...
        function deleteCommittee(item) {
            ban(item, '{{url('/admin/committee/delete')}}', {
                error_message: '{{trans('admin.general_error_message')}}',
        ...
        function restoreCommittee(item) {
            ban(item, '{{url('/admin/committee/restore')}}', {
                error_message: '{{trans('admin.general_error_message')}}',
                error_title: '{{trans('admin.error_title')}}',
